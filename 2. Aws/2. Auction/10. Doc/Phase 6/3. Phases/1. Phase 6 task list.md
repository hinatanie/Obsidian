Here are the tasks you need to complete for **Phase 6: Upload Auction Images using React, FastAPI, EC2, Lambda, S3, and RDS**. This list is based on your uploaded Phase 6 document, with Spring Boot replaced by FastAPI and with Lambda used only for suitable background image-processing work.


# Main Problem
Your Auction application needs to allow sellers to upload product images.
The target flow is:
```text
...
```text
auction-items/123/original/{uuid}.jpg
auction-items/123/thumbnails/{uuid}.webp
```
Checklist:
```text
[ ] Use UUIDs for file names
[ ] Do not use the original filename as the main key
[ ] Include the auction item ID in the object key
...
Resource scope:
```text
arn:aws:s3:::auction-product-images
arn:aws:s3:::auction-product-images/*
```
Checklist:
```text
[ ] Open the IAM Console
[ ] Create an S3 policy for auction-product-images
[ ] Allow PutObject
[ ] Allow GetObject
[ ] Allow DeleteObject
[ ] Allow ListBucket
...
Create or update:
```text
Role name: auction-backend-ec2-role
```
Attach the S3 policy from Task 4.
Checklist:
```text
[ ] Confirm auction-backend-ec2-role exists
...
Create:
```text
Role name: auction-image-lambda-role
```
The role may need:
```text
s3:GetObject
...
Checklist:
```text
[ ] Create auction-image-lambda-role
...
---
## Task 7: Create the `auction_images` Table
Use a separate table because one auction item can have multiple images.
...
Checklist:
```text
[ ] Create an AuctionImage SQLAlchemy model
[ ] Add auction_item_id
[ ] Add image_key
[ ] Add image_url
[ ] Add is_primary
...
S3_IMAGE_BUCKET=auction-product-images
IMAGE_MAX_SIZE_MB=5
```
You may later add:
```env
...
Create:
```text
POST /api/auction-items/{item_id}/images
```
Request:
```text
...
2. Find the auction item
3. Confirm the user owns the auction item
4. Validate the file
5. Upload the file to S3
6. Save metadata in auction_images
...
[ ] Confirm the auction item exists
...
Create:
```text
GET /api/auction-items/{item_id}/images
```
Return:
```json
[
...
[ ] Find images by auction item ID
[ ] Sort by sort_order
...
Create:
```text
DELETE /api/auction-items/{item_id}/images/{image_id}
```
The API must:
```text
1. Authenticate the user
2. Confirm the user owns the auction item
...
Create:
```text
PATCH /api/auction-items/{item_id}/images/{image_id}/primary
```
Logic:
```text
...
Checklist:
```text
[ ] Add images to the auction detail page
...
Create:
```text
Function name: auction-image-processor
Runtime: Python
Role: auction-image-lambda-role
```
The function can:
```text
...
Checklist:
```text
[ ] Create auction-image-processor
[ ] Select a supported Python runtime
[ ] Attach auction-image-lambda-role
...
```text
auction-items/*/original/
```
Checklist:
```text
[ ] Open the Lambda function
[ ] Add an S3 trigger
[ ] Select auction-product-images
...
```text
[ ] Attach Lambda to auction-vpc
[ ] Select the private subnets
[ ] Use auction-lambda-sg
[ ] Allow auction-lambda-sg in auction-rds-sg
...
[ ] 2. Create auction-product-images
[ ] 3. Enable Block Public Access
[ ] 4. Enable S3 encryption
[ ] 5. Design the S3 object-key structure
[ ] 6. Create the S3 IAM policy
[ ] 7. Attach the policy to auction-backend-ec2-role
[ ] 8. Create the auction_images database model
...
# Tasks to Do Later
```text
[ ] Create auction-image-processor Lambda
...
[ ] Do not let users upload images to another seller's auction
...
```text
Seller uploads an auction image
