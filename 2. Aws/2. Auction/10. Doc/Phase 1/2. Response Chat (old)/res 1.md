# Main Problem
While learning AWS, you may create resources for your Auction app and forget that they are still running.
...
In the AWS Console:
1. Sign in using your `auction-dev-admin` IAM user.
...
7. Configure:
```
Budget name: auction-learning-monthly-budget
Period: Monthly
...
## Solution
After working on your Auction backend:
1. Open **EC2**.
2. Select **Instances**.
3. Find your Auction instance.
4. Choose:
```
Instance state
→ Stop instance
...
## Solution
After testing your Auction database:
1. Open **RDS**.
2. Choose **Databases**.
3. Select your Auction database.
4. Choose:
```
Actions
→ Stop temporarily
...
## Solution
For your early Auction project, avoid creating a NAT Gateway unless the architecture genuinely requires one.
...
```
After every Auction app practice session:
```
Stop EC2
    ↓
Stop RDS
